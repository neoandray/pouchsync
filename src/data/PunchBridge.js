...
//Change to Mango query
   ...
   // bulkDocs(docs, options){
    syncDocs(docs, options){
       
        if(!options ){
            options = {}
        }
     if(!options.selector || Object.getOwnPropertyNames(options.selector).length ===0){
       let matchFilterList = []
        options.matchingFields.forEach((field)=>{
            matchFilterList.push( { 
                'field': field
                ,'value': docs.map((x)=>x[field])
                ,'operator':Selector.IN
            })

        })
        options['selector'] = new Selector(matchFilterList)
     }
    ...
    if(options && Object.getOwnPropertyNames(options).length >0)  {
       ...
       return this.find(options).then((foundDocs)=>{
            ...
            let  results = []
        // console.log("MongoDocs count: "+docs.length)
        // console.log("CouchDocs count: "+foundDocs["docs"].length)
            //console.dir(foundDocs["results"][0]['docs'])

                    if(foundDocs && ('docs' in foundDocs)){ 

                        let docIdSet         = docs.map( (x)=> (x._id))
                        
                        removedIDSet         = foundDocs["docs"].filter(
                        (y)=>{ 
                            return (docIdSet.indexOf(y['_id'])<0) 
                            }).map((x)=>({
                            '_id': x['_id'],'_rev':x['_rev'], '_deleted':true 
                            } )
                        );
                        // console.log(`docIdSet: ${docIdSet}`)
                        // console.log(`removedIDSet: ${JSON.stringify(removedIDSet)}`)
                        ...
                        docs.forEach((mongoDoc)=> {
                                ...
                                let savedDoc = couchDBDocs.filter((x)=>{return x[options.matchingFields[0]] ===  mongoDoc[options.matchingFields[0]] })
                                if (savedDoc.length===0){ 
                                    ...
                                    let tempSyncFields    =  options.watchedFields?options.watchedFields.map((f)=>{return f.toLowerCase()}):null
